        ...
        for col_index in range(num_columns):
            if control_locations[row_index][col_index] > 0:
                ...
                while total_controls == 0:
                    dist += 1
                    relevant_locations = control_locations[
                        max(0, row_index - dist):min(num_rows, row_index + dist + 1),
                        max(0, col_index - dist):min(num_columns, col_index + dist + 1),
                    ]
                    total_controls = relevant_locations.sum()

                relevant_plate = plate_array[
                    max(0, row_index - dist):min(num_rows, row_index + dist + 1),
                    max(0, col_index - dist):min(num_columns, col_index + dist + 1),
                ]
                control_values = (np.multiply(relevant_plate, relevant_locations)).sum()
                normalized_plate[row_index][col_index] = (
                    plate_array[row_index][col_index] * 100 * total_controls / control_values
                )
...
def normalize_plate_column_effect(plate_array, layout, neg_control_id, min_dist=1):
    ...
    for col_index in range(num_columns):
        ...
        for row_index in range(num_rows):
            normalized_plate[row_index][col_index] = (
                plate_array[row_index][col_index] * normalization_value
            )
...
def normalize_plate_striped_even_rows_column_effect(plate_array, layout, neg_control_id, min_dist=1):
    ...
    for col_index in range(num_columns):
        ...
        while total_controls == 0:
            ...
            relevant_locations = np.array([
                [relevant_locations[i, j] if i % 2 == 0 else 0 for j in range(y)]
                for i in range(x)
            ])
            total_controls = relevant_locations.sum()
...
def normalize_plate_striped_odd_rows_column_effect(plate_array, layout, neg_control_id, min_dist=1):
    ...
    for col_index in range(num_columns):
        ...
        while total_controls == 0:
            ...
            x, y = relevant_locations.shape
            relevant_locations = np.array([
                [relevant_locations[i, j] if i % 2 == 1 else 0 for j in range(y)]
                for i in range(x)
            ])
            total_controls = relevant_locations.sum()
...
def normalize_plate_lowess_deprecated(plate_array, layout, neg_control_id=None, min_dist=None, my_frac=1.0):
    ...
    lowess_rows_model.fit(
        y_adjusted.loc[y_adjusted['Type'] == neg_control_id, ['Rows']].to_numpy(),
        y_adjusted[y_adjusted.Type == neg_control_id].Intensity.to_numpy(),
        frac=my_frac, num_fits=5000,
    )
    xnew_rows = np.arange(num_rows)
    y_pred_rows = lowess_rows_model.predict(xnew_rows)

    y_adjusted.loc[y_adjusted['Type'] > 0, ['Intensity']] -= (
        y_pred_rows[y_adjusted.loc[y_adjusted['Type'] > 0, ['Rows']]]
    )
    y_adjusted.loc[y_adjusted['Type'] > 0, ['Intensity']] += np.nanmean(y_pred_rows)

    lowess_model = lowess.Lowess()
    lowess_model.fit(
        y_adjusted[y_adjusted.Type == neg_control_id].Columns.to_numpy(),
        y_adjusted[y_adjusted.Type == neg_control_id].Intensity.to_numpy(),
        frac=my_frac, num_fits=5000,
    )
    xnew = np.arange(num_columns)
    y_pred = lowess_model.predict(xnew)

    y_adjusted.loc[y_adjusted['Type'] > 0, ['Intensity']] -= (
        y_pred[y_adjusted.loc[y_adjusted['Type'] > 0, ['Columns']]]
    )
    y_adjusted.loc[y_adjusted['Type'] > 0, ['Intensity']] += np.nanmean(y_pred)

    unstack_adjusted_df = y_adjusted[["Rows", "Columns", "Intensity"]].copy()
    unstacked_adjusted_df = pd.pivot_table(
        unstack_adjusted_df, values='Intensity', index=['Rows'], columns=['Columns'], aggfunc='sum'
    )

    return normalize_plate_mean(unstacked_adjusted_df.to_numpy(), layout, neg_control_id)


# This is the preferred normalisation method.
def normalize_plate_lowess_2d(plate_array_in, layout, neg_control_id, min_dist=None, frac=1.0):
    """Smooth a plate using 2-D LOESS on the log10-transformed intensities.

    Bug fix vs. original: when fewer than 2 control wells survive (e.g. after
    row loss), the fallback now returns plate_array_in (original scale) instead
    of plate_array (log10 scale), which was an off-by-one scale error.
    """
    ...
    if control_locations.sum() < 2:
        # FIX: return original-scale plate, not the log10-transformed one.
    ...
    with warnings.catch_warnings():
        ...
        zout, _ = loess_2d(x, y, z, xnew=xnew, ynew=ynew, degree=1, frac=frac,
                           npoints=None, rescale=False, sigz=None)
        zout_controls, _ = loess_2d(x, y, z, degree=1, frac=frac,
                                    npoints=None, rescale=False, sigz=None)
...
def normalize_plate_linear(plate_array_in, layout, neg_control_id, min_dist=None):
    ...
    if control_locations.sum() < 2:
        # FIX: return original-scale plate, not the log10-transformed one
        # (same fix as normalize_plate_lowess_2d).
        return plate_array_in.copy()
    ...
    with warnings.catch_warnings():
        ...
        linear_model_rows.fit(
            y_adjusted[y_adjusted.Type == neg_control_id].Rows.to_numpy().reshape(-1, 1),
            y_adjusted[y_adjusted.Type == neg_control_id].Intensity.to_numpy().reshape(-1, 1),
        )
    ...
    y_adjusted.loc[y_adjusted['Type'] > 0, ['Intensity']] -= (
        linear_model_rows.predict(y_adjusted.loc[y_adjusted['Type'] > 0, ['Rows']].values)
    )
    ...
    with warnings.catch_warnings():
        ...
        linear_model_columns.fit(
            y_adjusted[y_adjusted.Type == neg_control_id].Columns.to_numpy().reshape(-1, 1),
            y_adjusted[y_adjusted.Type == neg_control_id].Intensity.to_numpy().reshape(-1, 1),
        )

    xnew_columns = np.arange(num_columns)
    y_pred_columns = linear_model_columns.predict(xnew_columns.reshape(-1, 1))
    mean_y = np.nanmean(y_pred_columns)

    y_adjusted.loc[y_adjusted['Type'] > 0, ['Intensity']] -= (
        linear_model_columns.predict(y_adjusted.loc[y_adjusted['Type'] > 0, ['Columns']].values)
    )
    y_adjusted.loc[y_adjusted['Type'] > 0, ['Intensity']] += mean_y
...
# TODO(dead-code): z_score is incomplete and never called by benchmark scripts.
...
def z_score(df):
    ...
    for col_index in range(num_columns):
        df_std[:, col_index] = (
            (df_std[:, col_index] - df_std[:, col_index].mean()) / df_std[:, col_index].std()
        )
    return df_std
